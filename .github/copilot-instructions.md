...
- The active Node.js application is [the HPA load console](../apps/hpa-console/README.md); run `npm --prefix apps/hpa-console test` for application changes. Its deployment uses public Node plus ConfigMaps, not an ACR build. The old `apps/scaling-demo` sources and `labs/01-scaling` were removed; preserve those deletions. Validate YAML schemas, embedded shell and PowerShell syntax, and Markdown references. Distinguish offline validation from live lab execution, and report whether Azure checks were actually run.